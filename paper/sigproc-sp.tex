...
\title{An automated hierarchical clustering algorithm for continuous high dimensional datasets
...
% 3rd. author
\alignauthor Siu-Wing Cheng\titlenote{This author is the
...
In many applications, a transformation to internal coordinates is more meaningful and easier to work with. For example, a 2D pendulum is naturally described by a single angle. A protein's primary degrees of freedom lie within the torsion angles. The internal coordinate space may often be in \(R^n \times C^m\) topological space. Our use of the parameter estimation allow us to effectively find the minima that correspond to a very low marginalized probability.


\begin{algorithm}
\caption{Assign points using EM}\label{euclid}
\begin{algorithmic}[1]
...
There are several nice properties afforded by this algorithm. Unlike the \(k\)-based algorithms, there is no need to explicitly specify the number of cluster centers a priori. Each level in the hierarchical cluster tree takes at most \(O(Nd\) time to process, and the further you go down the tree, the fewer the points. There is no possibility to oversplit a given cluster, so for hard systems like the double circle density, it will simply report that it has one cluster. The resulting clusters can individually serve as input to other types of clustering algorithms such as DBSCAN individually, or be used collectively as an initial configuration for the k* based types of clustering algorithms. The resulting probability curves derived via EM allows one to estimate the highest density point in any given cluster if one can assume independence in the dimensions.
 
Admittedly, there are also several caveats to this algorithm. Firstly, it relies on knowledge that the marginal probably density along some dimension has distinct modes. However, in many physics based applications

\begin{itemize}
\item There is no need to specify the number of centers a priori
\item Each level in the BFS tree takes at most \(O(Nd)\) time to process
\item No over splitting of a single cluster
\item Works effectively for periodic and nonperiodic systems
\item No need to explicitly define a metric between two points
\end{itemize}
